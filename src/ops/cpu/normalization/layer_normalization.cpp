...
namespace ops{
    ...
    std::shared_ptr<TensorImpl> LayerNormalization::forward(const std::vector<std::shared_ptr<TensorImpl>>& operands) {
        ...
        try {
            ...
            auto lnorm_pd = dnnl::layer_normalization_forward::primitive_desc(eng,
            ...
            std::unique_ptr<dnnl::memory> mean_mem (new dnnl::memory(lnorm_pd.mean_desc(), eng));
            std::unique_ptr<dnnl::memory> variance_mem (new dnnl::memory(lnorm_pd.variance_desc(), eng));

...
namespace ops{
    ...
    int64_t Linear::count = 0;

    Linear::Linear(float alpha, float beta, bool inc_counter)
    {
        if(inc_counter){
            m_name = "Linear"+std::to_string(count);
            count++;
        }
    ...
    std::shared_ptr<TensorImpl> Linear::forward(const std::vector<std::shared_ptr<TensorImpl>>& operands) {
        ...
        dnnl::eltwise_forward::primitive_desc Linear_desc(
            eng, 
        ...
        if ( in_tensor->requires_grad() ){
            requires_grad = true; 
            grad_fn = std::make_shared<Linear>(m_alpha, m_beta, true);
    ...
    void Linear::backward(const std::shared_ptr<TensorImpl>& diff_loss_out){

...
namespace ops{
    ...
    int64_t Relu::count = 0;

    Relu::Relu(float alpha, bool inc_counter ):
    m_alpha(alpha)
    {
        if(inc_counter){
            m_name = "Relu"+std::to_string(count);
            count++;
        }
    } 
 
    std::shared_ptr<TensorImpl> Relu::forward(const std::vector<std::shared_ptr<TensorImpl>>& operands) {
        ...
        dnnl::eltwise_forward::primitive_desc relu_desc(
            eng, 
        ...
        if ( in_tensor->requires_grad() ){
            requires_grad = true; 
            grad_fn = std::make_shared<Relu>(m_alpha,true);
    ...
    void Relu::backward(const std::shared_ptr<TensorImpl>& diff_loss_out){

...
namespace ops{
    ...
    int64_t Clip::count = 0;

    Clip::Clip(float alpha, float beta, bool inc_counter)
    {
        if(inc_counter){
            m_name = "Clip"+std::to_string(count);
            count++;
        }

        if (alpha > beta){
            throw std::invalid_argument("error: Clip() alpha must be less than or equal to beta");
        }
    ...
    std::shared_ptr<TensorImpl> Clip::forward(const std::vector<std::shared_ptr<TensorImpl>>& operands) {
        ...
        dnnl::eltwise_forward::primitive_desc Clip_desc(
            eng, 
        ...
        if ( in_tensor->requires_grad() ){
            requires_grad = true; 
            grad_fn = std::make_shared<Clip>(m_alpha, m_beta, true);
    ...
    void Clip::backward(const std::shared_ptr<TensorImpl>& diff_loss_out){

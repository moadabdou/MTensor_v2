...
namespace ops{
    ...
    Abs::Abs(bool inc_counter )
    {
        if(inc_counter){
            m_name = "Abs"+std::to_string(count);
            count++;
        }
    } 
 
    std::shared_ptr<TensorImpl> Abs::forward(const std::vector<std::shared_ptr<TensorImpl>>& operands) {
        ...
        dnnl::eltwise_forward::primitive_desc Abs_desc(
            eng, 
        ...
        if ( in_tensor->requires_grad() ){
            requires_grad = true; 
            grad_fn = std::make_shared<Abs>(true);
    ...
    void Abs::backward(const std::shared_ptr<TensorImpl>& diff_loss_out){

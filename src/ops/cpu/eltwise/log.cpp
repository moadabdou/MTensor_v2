...
namespace ops{
    ...
    Log::Log(bool inc_counter )
    {
        if(inc_counter){
            m_name = "Log"+std::to_string(count);
            count++;
        }
    } 
 
    std::shared_ptr<TensorImpl> Log::forward(const std::vector<std::shared_ptr<TensorImpl>>& operands) {
        ...
        dnnl::eltwise_forward::primitive_desc log_desc(
            eng, 
        ...
        if ( in_tensor->requires_grad() ){
            requires_grad = true; 
            grad_fn = std::make_shared<Log>(true);
    ...
    void Log::backward(const std::shared_ptr<TensorImpl>& diff_loss_out){

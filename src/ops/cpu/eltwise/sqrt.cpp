...
namespace ops{
    ...
    int64_t Sqrt::count = 0;

    Sqrt::Sqrt(bool inc_counter )
    {
        if(inc_counter){
            m_name = "Sqrt"+std::to_string(count);
            count++;
        }
    } 
 
    std::shared_ptr<TensorImpl> Sqrt::forward(const std::vector<std::shared_ptr<TensorImpl>>& operands) {
        ...
        dnnl::eltwise_forward::primitive_desc sqrt_desc(
            eng, 
        ...
        if ( in_tensor->requires_grad() ){
            requires_grad = true; 
            grad_fn = std::make_shared<Sqrt>(true);
    ...
    void Sqrt::backward(const std::shared_ptr<TensorImpl>& diff_loss_out){

...
namespace ops{
    ...
    Pow::Pow(float alpha, float beta, bool inc_counter)
    {
        if(inc_counter){
            m_name = "Pow"+std::to_string(count);
            count++;
        }
    ...
    std::shared_ptr<TensorImpl> Pow::forward(const std::vector<std::shared_ptr<TensorImpl>>& operands) {
        ...
        dnnl::eltwise_forward::primitive_desc Pow_desc(
            eng, 
        ...
        if ( in_tensor->requires_grad() ){
            requires_grad = true; 
            grad_fn = std::make_shared<Pow>(m_alpha, m_beta, true);
    ...
    void Pow::backward(const std::shared_ptr<TensorImpl>& diff_loss_out){

...
namespace ops{
    ...
    int64_t Sigmoid::count = 0;

    Sigmoid::Sigmoid(bool inc_counter )
    {
        if(inc_counter){
            m_name = "Sigmoid"+std::to_string(count);
            count++;
        }
    } 
 
    std::shared_ptr<TensorImpl> Sigmoid::forward(const std::vector<std::shared_ptr<TensorImpl>>& operands) {
        ...
        dnnl::eltwise_forward::primitive_desc sigmoid_desc(
            eng, 
        ...
        if ( in_tensor->requires_grad() ){
            requires_grad = true; 
            grad_fn = std::make_shared<Sigmoid>(true);
    ...
    void Sigmoid::backward(const std::shared_ptr<TensorImpl>& diff_loss_out){

...
namespace ops{
    ...
    Eq::Eq(bool inc_counter )
    {
        if(inc_counter){
            m_name = "Eq"+std::to_string(count);
            count++;
        }
    } 
 
    std::shared_ptr<TensorImpl> Eq::forward(const std::vector<std::shared_ptr<TensorImpl>>& operands) {
        ...
        try{
            ...
            if ( in_tensor_0->requires_grad() || in_tensor_1->requires_grad()){
                requires_grad = true; 
                grad_fn = std::make_shared<Eq>(true);
            ...
            throw std::runtime_error(
                std::string("error : Eq() ") + e.what()
            );
        }

    }  

    void Eq::backward(const std::shared_ptr<TensorImpl>& diff_loss_out){
//if (diff_loss_out->requires_grad()) std::cout << m_name ; 
        throw std::runtime_error("Backward is not implemented for eq() '=' ");
    }

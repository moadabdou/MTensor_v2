...
namespace ops{
        ...
        bool inc_counter)
    {
        ...
        if (strides.size() != 2 || strides[0] < 0 || strides[1] < 0){
            throw std::invalid_argument("error : Deconv2d() invalid strides were given");
        }
        if (padding_l.size() != 2 || padding_l[0] < 0 || padding_l[1] < 0){
            throw std::invalid_argument("error : Deconv2d() invalid padding_l was given");
        }
        if (padding_r.size() != 2 || padding_r[0] < 0 || padding_r[1] < 0){
            throw std::invalid_argument("error : Deconv2d() invalid padding_r was given");
        }
        ...
        if(inc_counter){
            m_name = "Deconv2d"+std::to_string(count);
            count++;
        }
    } 
 
    std::shared_ptr<TensorImpl> Deconv2d::forward(const std::vector<std::shared_ptr<TensorImpl>>& operands) {
        ...
        try {
            ...
            const auto OH = (src_shape[2] - 1 ) * m_strides[0] - m_padding_l[0] -  m_padding_r[0] + weights_shape[2];
            const auto OW = (src_shape[3] - 1 ) * m_strides[1] - m_padding_l[1] -  m_padding_r[1] + weights_shape[3];
            ...
            std::shared_ptr<Deconv2d> grad_fn = nullptr;
            bool requires_grad = false;

            if ( in_tensor->requires_grad() ){
                requires_grad = true; 
                grad_fn = std::make_shared<Deconv2d>(
                    m_strides,
                    m_padding_l,
                    m_padding_r,
                    true
            ...
            throw std::invalid_argument(std::string("error: Deconv2d() was not possible for in_tensor: ") + e.what());

        }
    }  

    void Deconv2d::backward(const std::shared_ptr<TensorImpl>& diff_loss_out){
        ...
        if (x->requires_grad()){
            ...
            auto conv_bwd = dnnl::deconvolution_backward_data(bwd_pd);
        ...
        if (w->requires_grad() || b && b->requires_grad()){
            ...
            auto deconv_bwd = dnnl::deconvolution_backward_weights(bwd_pd);
            deconv_bwd.execute(engine_stream, {
                {DNNL_ARG_DIFF_DST, diff_dst_mem},
                {DNNL_ARG_SRC, x_mem},

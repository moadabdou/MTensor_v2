...
namespace ops{
    ...
    {
        if (kernel.size() != 2 || kernel[0] < 0 || kernel[1] < 0){
            throw std::invalid_argument("error : AvgPooling2d() invalid kernel was given");
        }
        if (strides.size() != 2 || strides[0] < 0 || strides[1] < 0){
            throw std::invalid_argument("error : AvgPooling2d() invalid strides were given");
        }
        if (padding_l.size() != 2 || padding_l[0] < 0 || padding_l[1] < 0){
            throw std::invalid_argument("error : AvgPooling2d() invalid padding_l was given");
        }
        if (padding_r.size() != 2 || padding_r[0] < 0  || padding_r[1] < 0){
            throw std::invalid_argument("error : AvgPooling2d() invalid padding_r was given");
        }
        m_kernel = kernel;
        m_strides = strides;
        ...
        if(inc_counter){
            m_name = "AvgPooling2d"+std::to_string(count);
            count++;
        }
    } 
 
    std::shared_ptr<TensorImpl> AvgPooling2d::forward(const std::vector<std::shared_ptr<TensorImpl>>& operands) {
        ...
        try {
            ...
            if (in_tensor->shape().size() != 4 ){
                throw std::invalid_argument(" in_tensor must be of shape (B,C,H,W) ");
            }
            ...
            std::shared_ptr<AvgPooling2d> grad_fn = nullptr;
            bool requires_grad = false;

            if ( in_tensor->requires_grad() ){
                requires_grad = true; 
                grad_fn = std::make_shared<AvgPooling2d>(
                    m_kernel,
                    m_strides,
                    m_padding_l,
                    m_padding_r,
            ...
            throw std::invalid_argument(std::string("error: AvgPooling2d() was not possible for in_tensor: ") + e.what());

        }
    }  

    void AvgPooling2d::backward(const std::shared_ptr<TensorImpl>& diff_loss_out){

...
namespace ops{
    ...
    int64_t Mul_reduction::count = 0;

    Mul_reduction::Mul_reduction(const std::vector<int64_t>& dims, bool inc_counter)
    {
        if (*std::min_element(dims.begin(), dims.end()) < 0){
            throw std::invalid_argument("error : Mul_reduction() invalid dims are given");
        }
        ...
        if(inc_counter){
            m_name = "Mul_reduction"+std::to_string(count);
            count++;
        }
    } 
 
    std::shared_ptr<TensorImpl> Mul_reduction::forward(const std::vector<std::shared_ptr<TensorImpl>>& operands) {
        ...
        if (max_allowed_dim >= in_shape.size()){
            throw std::invalid_argument("error: Mul_reduction() dims are out of range of in_tensor dims");
        }
        ...
        if ( in_tensor->requires_grad() ){
            requires_grad = true; 
            grad_fn = std::make_shared<Mul_reduction>(m_dims, true);

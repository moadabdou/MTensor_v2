...
namespace ops{
    ...
    int64_t Norm_lp_power_p_sum::count = 0;

    Norm_lp_power_p_sum::Norm_lp_power_p_sum(const std::vector<int64_t>& dims,float p ,float eps , bool inc_counter):
    m_eps(eps),m_p(p)
    {
        if (*std::min_element(dims.begin(), dims.end()) < 0){
            throw std::invalid_argument("error : Norm_lp_power_p_sum() invalid dims are given");
        }
        ...
        if(inc_counter){
            m_name = "Norm_lp_power_p_sum"+std::to_string(count);
            count++;
        }
    } 
 
    std::shared_ptr<TensorImpl> Norm_lp_power_p_sum::forward(const std::vector<std::shared_ptr<TensorImpl>>& operands) {
        ...
        if (max_allowed_dim >= in_shape.size()){
            throw std::invalid_argument("error: Norm_lp_power_p_sum() dims are out of range of in_tensor dims");
        }
        ...
        if ( in_tensor->requires_grad() ){
            requires_grad = true; 
            grad_fn = std::make_shared<Norm_lp_power_p_sum>(m_dims,m_p, m_eps, true);

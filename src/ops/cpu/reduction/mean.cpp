...
namespace ops{
    ...
    Mean::Mean(const std::vector<int64_t>& dims, float eps, bool inc_counter):
    m_eps(eps)
    {
        if (*std::min_element(dims.begin(), dims.end()) < 0){
            throw std::invalid_argument("error : Mean() invalid dims are given");
        }
    ...
    std::shared_ptr<TensorImpl> Mean::forward(const std::vector<std::shared_ptr<TensorImpl>>& operands) {
        ...
        if (max_allowed_dim >= in_shape.size()){
            throw std::invalid_argument("error: Mean() dims are out of range of in_tensor dims");
        }
        ...
        if ( in_tensor->requires_grad() ){
            requires_grad = true; 
            grad_fn = std::make_shared<Mean>(m_dims, m_eps, true);

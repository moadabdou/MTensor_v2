...
TEST(deconv , 1d){
    ...
    std::shared_ptr<mt::TensorImpl> t2 = mt::TensorImpl::randn({1,1,5}, 0.0f,1.0f, 42, true);
    std::shared_ptr<mt::TensorImpl> t3 = mt::TensorImpl::ones({1,1,3}, true);
    std::shared_ptr<mt::TensorImpl> t4 = mt::TensorImpl::ones({64},true); // 9  
    std::cout << t1 << t3 ;
    std::cout << deconv1d.forward({t1,t3});
}
...
int main(int argc, char **argv) {
    ...
    ::testing::GTEST_FLAG(filter) = "deconv.3d";

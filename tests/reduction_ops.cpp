...
TEST(reduction_ops , all){
    ...
    mt::ops::Mean mean({0,2});
    mt::ops::Max_reduction max_reduction({0,2});
    mt::ops::Min_reduction min_reduction({0,2});
    ...
    std::shared_ptr<mt::TensorImpl> t1 = mt::TensorImpl::randn({3,2,1,2}, 0.0f, 2.0f, true);
    std::shared_ptr<mt::TensorImpl> t2 = e.forward({s.forward({t1})});
    
    std::cout << t1;
    std::cout << t2;
    std::cout << mean.forward({t2});
    std::cout << max_reduction.forward({t2});
